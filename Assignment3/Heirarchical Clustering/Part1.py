...
class Cluster:
	...
	def fit(self, X):
		...
		while(i > 1):
			...
			elem1.append(list(set(elem1[index1]).union(elem1[index2])))
			del elem1[index1]
			if(index1 < index2):
				del elem1[index2-1]
			else:
...
# print(clf.compute_jaccard_coef(['Clustering', 'Supervised', 'Machine Learning', 'AI'],['Clustering','Unsupervised', 'Machine Learning', 'HCI']))
clf.fit(topics)

for i in range(len(clf.cluster)):
	print(str(i+1) + ":")
	print(clf.cluster[i+1][0])
	print("")
